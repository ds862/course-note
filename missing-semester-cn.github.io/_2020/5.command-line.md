...
如果我们向这个程序发送两次 `SIGINT` ，然后再发送一次 `SIGQUIT`（CTRL \），程序会有什么反应？注意 `^` 是我们在终端输入`Ctrl` 时的表示形式：
...
我们可以使用 [`fg`](https://www.man7.org/linux/man-pages/man1/fg.1p.html) (foreground)或 [`bg`](http://man7.org/linux/man-pages/man1/bg.1p.html)(background) 命令恢复暂停的工作。它们分别表示在前台继续或在后台继续。
...
自定义tmux：https://www.hamvocke.com/blog/a-guide-to-customizing-your-tmux-conf/
...
我们应该如何管理这些配置文件呢，它们应该在它们的文件夹（dotfiles）下，并使用版本控制系统进行管理，然后通过脚本将其 **符号链接** 到需要的地方。这么做有如下好处：
